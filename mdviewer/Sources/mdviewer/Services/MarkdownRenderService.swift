...
actor MarkdownRenderService: MarkdownRendering {
    ...
    func render(_ request: RenderRequest) -> RenderedMarkdown {
        ...
        if let cached = cache.object(forKey: cacheKey) {
            stats.cacheHits += 1
            logger.debug("Cache hit for key: \(cacheKey.substring(to: min(8, cacheKey.length)), privacy: .public)...")
            return cached
        }
        stats.cacheMisses += 1
        ...
        logger.debug("Rendered markdown chars=\(request.markdown.count, privacy: .public) elapsedMs=\(elapsedMs, privacy: .public) cacheSize=\(cost, privacy: .public)")
