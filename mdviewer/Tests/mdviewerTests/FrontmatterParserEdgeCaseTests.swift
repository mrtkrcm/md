...
final class FrontmatterParserEdgeCaseTests: XCTestCase {
    ...
    func testParsesEmptyFrontmatterBlock() {
        ...
        XCTAssertTrue(parsed.frontmatter?.entries.isEmpty ?? false,
    ...
    func testParsesYamlCommentLines() {
        ...
        XCTAssertEqual(parsed.frontmatter?.entries.count, 1,
    ...
    func testOnlyFirstFrontmatterBlockIsParsed() {
        ...
        XCTAssertTrue(parsed.renderedMarkdown.contains("Second Block") || parsed.renderedMarkdown.contains("More body"),
    ...
    func testFrontmatterImmediatelyFollowedByCodeFence() {
        ...
        XCTAssertTrue(parsed.renderedMarkdown.contains("let x = 42"),
    ...
    func testDuplicateKeysLastValueWins() {
        ...
        """
        let parsed = FrontmatterParser.parse(markdown)
        XCTAssertEqual(parsed.frontmatter?.metadata["title"], "Second Title",
        ...
        XCTAssertEqual(titleEntries.count, 1,
    ...
    func testMultiLineHtmlCommentIsStripped() {
        ...
        """
        let parsed = FrontmatterParser.parse(markdown)
        XCTAssertFalse(parsed.renderedMarkdown.contains("multiple lines"),
            "Multi-line HTML comment content should be stripped")
        XCTAssertTrue(parsed.renderedMarkdown.contains("# Heading After Comment"),
    ...
    func testHtmlCommentInsideCodeFenceIsNotStripped() {
        ...
        """
        let parsed = FrontmatterParser.parse(markdown)
        XCTAssertTrue(parsed.renderedMarkdown.contains("<!-- this comment is inside a code fence -->"),
    ...
    func testTildeFencePreservesHtmlComment() {
        ...
        """
        let parsed = FrontmatterParser.parse(markdown)
        XCTAssertTrue(parsed.renderedMarkdown.contains("<!-- comment inside tilde fence -->"),
    ...
    func testInlineHtmlCommentOnSameLineIsStripped() {
        ...
        """
        let parsed = FrontmatterParser.parse(markdown)
        XCTAssertFalse(parsed.renderedMarkdown.contains("single line comment"),
            "Inline HTML comment on its own line should be stripped")
        XCTAssertTrue(parsed.renderedMarkdown.contains("# Heading After"),
    ...
    func testManyEntriesPreserveInsertionOrder() {
        ...
        XCTAssertEqual(keys, ["alpha", "bravo", "charlie", "delta", "echo", "foxtrot", "golf", "hotel"],
