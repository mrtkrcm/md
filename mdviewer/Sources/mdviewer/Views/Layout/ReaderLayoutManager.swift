...
final class ReaderLayoutManager: NSLayoutManager {
    ...
    override func drawBackground(forGlyphRange glyphsToShow: NSRange, at origin: NSPoint) {
        ...
        while i < charRange.location + charRange.length, i < totalLen {
            ...
            let intent = ts.attribute(MarkdownRenderAttribute.presentationIntent, at: i, effectiveRange: &effectiveRange)
            ...
            let bqDepth = ts.attribute(MarkdownRenderAttribute.blockquoteDepth, at: i, effectiveRange: nil) as? Int ?? 0
            ...
            if isCode, let bg = ts.attribute(.backgroundColor, at: i, effectiveRange: nil) as? NSColor {
                if let last = codeSpans.last, last.charEnd >= i {
                    ...
                    codeSpans.append(Span(charStart: effectiveRange.location, charEnd: end, kind: .code(bg: bg)))
                      ...
                      let bg     = ts.attribute(MarkdownRenderAttribute.blockquoteBackground, at: i, effectiveRange: nil) as? NSColor,
                      let accent = ts.attribute(MarkdownRenderAttribute.blockquoteAccent,     at: i, effectiveRange: nil) as? NSColor {
